...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Audience_final_data.csv", type=["csv"])
    if uploaded:
    ...
    uploaded2 = st.file_uploader("Survey CSV or Excel", type=["csv", "xlsx"], key="survey_upload")
    if uploaded2:
        raw = uploaded2.read()
...
with h2:
    st.markdown("""
    <div style='text-align:right;padding-top:14px;font-size:22px;font-weight:700;color:#222;font-style:italic;'>
      monkey baa
    </div>
    """, unsafe_allow_html=True)
